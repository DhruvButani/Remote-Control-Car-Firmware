...
/* TEMP: gated alongside DEBUG_ENABLE=false in task_console.h so SCB5 can
 * be freed for LCD SPI without this file's function definitions colliding
 * with the empty `task_debug_printf(...)` macro the header emits when
 * DEBUG_ENABLE is false. Flip DEBUG_ENABLE back to true to restore. */
#if (DEBUG_ENABLE)
...
#endif /* DEBUG_ENABLE - matches guard opened after #include "task_console.h" */
