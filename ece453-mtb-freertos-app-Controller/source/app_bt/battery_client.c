...
#include "app_hw/joystick.h"
#include "app_hw/buttons.h"
#include "app_hw/task_bt_send.h"
...
//Car service UUID
// #define UUID_SERVICE_CAR_DATA   0xFF202D108CCC425A9FE376B086A5C57F
...
// Discovered handles — zero until discovery populates them
static uint16_t car_data_out_val_handle = 0;
static uint16_t car_data_out_cccd_handle = 0;
static uint16_t car_data_in_val_handle = 0;

uint16_t car_data_in_get_val_handle(void)
{
    return car_data_in_val_handle;
}
...
static const uint8_t UUID_CAR_DATA_SERVICE[16] = {
    0x7F, 0xC5, 0xA5, 0x86, 0xB0, 0x76, 0xE3, 0x9F,
    0x5A, 0x42, 0xCC, 0x8C, 0x10, 0x2D, 0x20, 0xFF
};
// UUIDs in little-endian
static const uint8_t UUID_CAR_DATA_OUT[16] = {
    0xE7, 0x8A, 0x55, 0x54, 0x02, 0x62, 0x9C, 0xA7,
    0x54, 0x47, 0x2C, 0xD0, 0xD0, 0x00, 0x55, 0x6E
};

static const uint8_t UUID_CAR_DATA_IN[16] = {
    0xBD, 0x32, 0xB8, 0x3D, 0xEF, 0x0E, 0xB1, 0xA6,
    0x29, 0x4B, 0xED, 0x9C, 0xE6, 0x80, 0x87, 0xF4
};
    ...
    switch (event)
    {
        case BTM_ENABLED_EVT:
            ...
            if (WICED_BT_SUCCESS == p_event_data->enabled.status)
            {
                ...
                task_print_info("Local Bluetooth Address: ");
            ...
            {
                task_print_info( "Bluetooth enable failed \r\n" );
            }
            break;
        ...
        case BTM_USER_CONFIRMATION_REQUEST_EVT:
            task_print_info("Numeric_value: %"PRIu32" \r\n", p_event_data->user_confirmation_request.numeric_value);
        ...
        case BTM_PAIRING_COMPLETE_EVT:
            ...
            task_print_info( "Battery client, Pairing Complete Reason: %s ",
        ...
        case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
             ...
             {
                 task_print_info("Successfully Bonded to ");
                 print_bd_address(p_event_data->paired_device_link_keys_update.bd_addr);
             }
             else
             {
                 task_print_info("Failed to bond! \r\n");
             }
            break;

        case  BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
            /* Paired Device Link Keys Request */
            task_print_info("Paired Device Link keys Request Event for device ");
            ...
            {
                task_print_info("Device Link Keys not found in the database! \n");
                bondindex=0;
            }
            break;
        ...
        case BTM_ENCRYPTION_STATUS_EVT:
        {
            ...
            task_print_info("encryption status: res %d\n", p_status->result);
        }
            break;
        ...
        case BTM_BLE_CONNECTION_PARAM_UPDATE:
            task_print_info("Connection parameter update status:%d, Connection Interval: %d, Connection Latency: %d, Connection Timeout: %d\n",
        ...
        case BTM_BLE_PHY_UPDATE_EVT:
            task_print_info("Selected TX PHY - %dM\r\n Selected RX PHY - %dM\r\n",
        ...
        case BTM_BLE_SCAN_STATE_CHANGED_EVT:
            ...
            {
                task_print_info("Scanning stopped\n");
#ifdef PSOC6_BLE
            ...
            task_print_info( "Scan State Change: %d\r\n", p_event_data->ble_scan_state_changed );
            break;

        default:
            task_print_info("Unhandled Bluetooth Management Event: 0x%x %s\n", event, get_bt_event_name(event));
            break;
    }
...
static void battery_client_application_init(void)
{
    ...
    {
        //task_print("Keys found in flash, add them to Addr Res DB\r\n");
    ...
    task_print_info("GATT event Handler registration status: %s \n",get_bt_gatt_status_name(gatt_status));
    ...
    /* Prompt user */
    task_print("======================================================================\n");
    task_print("| Press and Hold User Button:                                         |\n");
    task_print("|   0-2  seconds: Start Scan and Pair with Battery Server.            |\n");
    task_print("|   2-5  seconds: Request a Read of Battery Level.                    |\n");
    task_print("|   5-10 seconds: Enable/Disable Battery Service.                     |\n");
#ifdef PSOC6_BLE
    task_print("|   >10  seconds:  Connect, Pair and Bond with a new peer device      |\n");
#endif
    task_print("======================================================================\n");
    ...
    /* Auto-start scan for the Car server — no button press required. */
    task_print("Auto-scanning for Car server...\n");
...
void button_task(void *arg)
{
    for(;;)
    {
        ...
        if (cyhal_gpio_read(CYBSP_USER_BTN) == USR_BUTTON_STATE_UP)
        {
            task_print("\n--- Button pressed: scanning for Car server ---\n");
            battery_client_enter_pairing();
        }
    }
}
...
static void battery_client_enable( wiced_bool_t enable )
{
    ...
    if(is_bas_enabled)
    {
        task_print("Enabling notifications\n");
        wiced_bt_battery_client_enable( battery_client_app_data.conn_id );
    }
    else
    {
        task_print("Disabling notifications\n");
...
{
    task_print("%s data: ", msg);
    while (len--)
    {
        task_print("%02x ",*p_data++);
    }
    task_print("\r\n");
}
...
static void battery_client_show_data(char * msg, wiced_bt_battery_client_event_t event, wiced_bt_battery_client_event_data_t *p_data)
{
    ...
    case UUID_CHARACTERISTIC_BATTERY_LEVEL:
        ...
        {
            task_print("[%13s] Battery Level:%d\r\n", msg, p_data->data.p_data[0]);
        }
        break;
    ...
    case UUID_CHARACTERISTIC_BATTERY_LEVEL_STATUS:
        {
            ...
            task_print("BATTERY_LEVEL_STATUS: flags8:%d  batt_present:%d wired_ext_pwr_connected:%d wireless_ext_pwr_connected:%d \
            ...
            task_print("identifier:%d  battery_level:%d service_required:%d rfu:%d\r\n\r\n", identifier, battery_level,
    ...
    case UUID_CHARACTERISTIC_BATTERY_ESTIMATED_SERVICE_DATE:
        {
            ...
            task_print("ESTIMATED_SERVICE_DATE:%d  %s \r\n\r\n", date, msg );
            break;
        }
    case UUID_CHARACTERISTIC_BATTERY_CRITICAL_STATUS:
        {
            ...
            task_print("CRITICAL_STATUS:%s critical_power_state:%d immediate_service_required:%d critical_status.rfu:%d\r\n\r\n",  msg,
    ...
    case UUID_CHARACTERISTIC_BATTERY_ENERGY_STATUS:
        {
            ...
            task_print("BATTERY_ENERGY_STATUS:%s flags8:%d external_source_power:%d present_voltage:%d \r\n\r\n",
                            msg, flags8, external_source_power, present_voltage);
            task_print("available_energy:%d  available_power_capacity:%d change_rate:%d  available_energy_at_last_change:%d \r\n",
    ...
    case UUID_CHARACTERISTIC_BATTERY_TIME_STATUS:
        {
            ...
            task_print("TIME_STATUS:%s flags8:%d time_until_discharged:%d time_until_discharged_on_standby:%d time_until_recharged:%d\r\n\r\n",
    ...
    case UUID_CHARACTERISTIC_BATTERY_HEALTH_STATUS:
        {
            ...
            task_print("HEALTHSTATUS:%s flags16:%d health_summary:%d cycle_count:%d current_temp:%d deep_discharge_count:%d \r\n\r\n", msg, flags16,
    ...
    case UUID_CHARACTERISTIC_BATTERY_HEALTH_INFO:
        {
            ...
            task_print("HEALTH_STATUS:%s flags:%d cycle_count :%d min__op_temp:%d max_op_temp:%d \r\n\r\n", msg,
    ...
    case UUID_CHARACTERISTIC_BATTERY_INFO:
        {
            ...
            task_print("BATTERY INFO:[%5s] flags16:%d battery_features:%02X battery_replace:%d battery_recharge:%d batt_features_rfu:%d \r\n",
                            msg, flags16, features, batt_features->batt_replaceable, batt_features->batt_rechargeable, batt_features->rfu);
            task_print("batt_manuf_date:%d batt_expr_date:%d  batt_designed_cap:%d batt_low_energy:%d batt_critical_energy:%d \r\n",
                            batt_manuf_date, batt_expr_date, batt_designed_cap, batt_low_energy, batt_critical_energy);
            task_print("batt_chemistry:%d batt_nominal_volage:%d batt_aggr_grp:%d \r\n\r\n",
...
static void battery_client_callback(wiced_bt_battery_client_event_t event, wiced_bt_battery_client_event_data_t *p_data)
{
    ...
    case WICED_BT_BAC_EVENT_DISCOVERY_COMPLETE:
        ...
        {
            task_print("Battery Level Service not found!\n");
        }
        break;
    ...
    default:
        task_print("Unknown BAC Event:%d\r\n", event);
        break;
    }
}
...
void battery_client_scan_result_cback( wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data )
{
    ...
    if ( p_scan_result )
    {
        ...
        if(p_data != NULL && length >= 16 && memcmp(p_data, UUID_CAR_DATA_SERVICE, 16) == 0) {
            // found our server
            ...
            // if ( p_data == NULL )
            // {
            //     p_data = wiced_bt_ble_check_advertising_data( p_adv_data, BTM_BLE_ADVERT_TYPE_128SRV_PARTIAL, &length );
            ...
            //     if (service_uuid16 == UUID_SERVICE_CAR_DATA)
            //     {
            ...
            task_print( "wiced_bt_gatt_connect status %d\r\n", ret_status );
        }
...
static void battery_client_enter_pairing()
{
    ...
    if (( battery_client_app_data.conn_id == 0 ) && (wiced_bt_ble_get_current_scan_state() == BTM_BLE_SCAN_TYPE_NONE))
    {
        ...
        task_print("wiced_bt_ble_scan: %d \r\n", result);
    }
    else
    {
         task_print("Scanning not initiated, scan in progress ??  \r\n");
    }
}
...
static wiced_bt_gatt_status_t battery_client_connection_up( wiced_bt_gatt_connection_status_t *p_conn_status )
{
    ...
    task_print("battery_client_connection_up()\n");
    ...
    task_print( "battery client_connection_up Conn Id:%d ",p_conn_status->conn_id);
    ...
    task_print("start discover status:%d\r\n", status);
...
{
    task_print("battery client connection down\r\n");
    ...
    car_data_out_val_handle  = 0;
    car_data_out_cccd_handle = 0;
    car_data_in_val_handle   = 0;
...
/*******************************************************************************
* Function Name: car_data_client_discovery_result (replacing wiced_bt_battery_client_discovery_result)
********************************************************************************/
void car_data_client_discovery_result(wiced_bt_gatt_discovery_result_t *p_data)
{
    ...
    if ((p_data->discovery_type == GATT_DISCOVER_CHARACTERISTICS) &&
        (p_char->char_uuid.len == LEN_UUID_128))
    {
        if (memcmp(p_char->char_uuid.uu.uuid128, UUID_CAR_DATA_OUT, 16) == 0)
        {
            car_data_out_val_handle = p_char->val_handle;
            task_print("Found CarDataOut handle:%04x val_handle:%04x\r\n", p_char->handle, p_char->val_handle);
        }
        else if (memcmp(p_char->char_uuid.uu.uuid128, UUID_CAR_DATA_IN, 16) == 0)
        {
            car_data_in_val_handle = p_char->val_handle;
            task_print("Found CarDataIn handle:%04x val_handle:%04x\r\n", p_char->handle, p_char->val_handle);
        }
    }
    else if ((p_data->discovery_type == GATT_DISCOVER_CHARACTERISTIC_DESCRIPTORS) &&
             (p_info->type.len == LEN_UUID_16))
    {
        // CCCD is always a standard 16-bit UUID (0x2902)
        if (p_info->type.uu.uuid16 == UUID_DESCRIPTOR_CLIENT_CHARACTERISTIC_CONFIGURATION)
        {
            // NOTE: This assumes descriptor discovery runs for CarDataOut.
            // If you discover descriptors for both characteristics, you'll
            // need to track which characteristic's descriptors are currently
            // being enumerated (similar to cur_char_idx in the original).
            car_data_out_cccd_handle = p_info->handle;
            task_print("Found CarDataOut CCCD handle:%04x\r\n", p_info->handle);
        }
    }
}
...
{
    task_print("[%s] conn %d, discovery type %d, state %d\r\n", __FUNCTION__, p_data->conn_id, p_data->discovery_type, battery_client_app_state.discovery_state);

    switch (battery_client_app_state.discovery_state)
    {
        case BAC_DISCOVERY_STATE_CHAR:
        case BAC_DISCOVERY_STATE_DESC:
            car_data_client_discovery_result(p_data);
            break;

        default:
            if (p_data->discovery_type  == GATT_DISCOVER_SERVICES_ALL)
            {
                if (p_data->discovery_data.group_value.service_type.len == LEN_UUID_128)
                {
                   
                   task_print("uuid:%04x start_handle:%04x end_handle:%04x\r\n",
                   ...
                   if(memcmp(p_data->discovery_data.group_value.service_type.uu.uuid128, UUID_CAR_DATA_SERVICE, LEN_UUID_128) == 0 && battery_service_found != 1)
                   {
                       battery_service_found = 1; // ignore any more battery services, connect to first discovered one
                       task_print("Car Data Service found s:%04x e:%04x\r\n",
                               p_data->discovery_data.group_value.s_handle,
                               p_data->discovery_data.group_value.e_handle);
...
wiced_bt_gatt_status_t battery_client_gatt_discovery_complete(wiced_bt_gatt_discovery_complete_t *p_data)
{
    ...
    task_print("[%s] conn %d type %d state %d\r\n", __FUNCTION__, p_data->conn_id, p_data->discovery_type, battery_client_app_state.discovery_state);

    switch (battery_client_app_state.discovery_state)
    {
    case BAC_DISCOVERY_STATE_SERVICE:
        /* Service discovery just finished */
        if (p_data->discovery_type == GATT_DISCOVER_SERVICES_ALL)
        {
            task_print("CarData service handles:%04x-%04x\r\n",
                   battery_client_app_state.bac_s_handle,
                   battery_client_app_state.bac_e_handle);

            if ((battery_client_app_state.bac_s_handle != 0) &&
                (battery_client_app_state.bac_e_handle != 0))
            {
                /* Start characteristic discovery within the service range */
                battery_client_app_state.discovery_state = BAC_DISCOVERY_STATE_CHAR;
                status = wiced_bt_util_send_gatt_discover(
                             p_data->conn_id,
                             GATT_DISCOVER_CHARACTERISTICS,
                             0,   /* no UUID filter — discover all chars */
                             battery_client_app_state.bac_s_handle,
                             battery_client_app_state.bac_e_handle);
                task_print("Start char discovery status:%d\r\n", status);
            }
            else
            {
                task_print("CarData service not found!\n");
            }
        }
        break;

    case BAC_DISCOVERY_STATE_CHAR:
        /* Characteristic discovery just finished — now discover descriptors
         * in the range between CarDataOut value handle and CarDataIn's
         * declaration (the CCCD lives in that gap). */
        if (car_data_out_val_handle != 0 && car_data_in_val_handle != 0)
        {
            battery_client_app_state.discovery_state = BAC_DISCOVERY_STATE_DESC;
            status = wiced_bt_util_send_gatt_discover(
                         p_data->conn_id,
                         GATT_DISCOVER_CHARACTERISTIC_DESCRIPTORS,
                         0,
                         car_data_out_val_handle + 1,
                         car_data_in_val_handle - 1);
            task_print("Start descriptor discovery status:%d\r\n", status);
        }
        else
        {
            task_print("CarDataOut or CarDataIn not found — cannot discover descriptors\n");
        }
        break;

    case BAC_DISCOVERY_STATE_DESC:
        /* Descriptor discovery just finished — subscribe to notifications */
        task_print("Discovery complete. CarDataOut val:0x%04X cccd:0x%04X  CarDataIn val:0x%04X\n",
               car_data_out_val_handle, car_data_out_cccd_handle, car_data_in_val_handle);

        car_data_subscribe_notifications(p_data->conn_id, car_data_out_cccd_handle);
        break;

    default:
        task_print("!!!! unexpected discovery_state:%d\r\n", battery_client_app_state.discovery_state);
        break;
    }
    return WICED_BT_GATT_SUCCESS;
}
...
wiced_bt_gatt_status_t battery_client_gatt_operation_complete(wiced_bt_gatt_operation_complete_t *p_data)
{
    ...
    //task_print("battery_client_gatt_operation_complete conn %d op %d st %d\r\n", p_data->conn_id, p_data->op, p_data->status );
    ...
    if ( p_data->status == WICED_BT_GATT_INSUF_AUTHENTICATION )
    {
        ...
        else
        {
            ...
            task_print( "wiced_bt_dev_sec_bond %d \r\n", status );
        }
    }
...
{
    /* CarDataOut is notify-only in normal operation.
     * If a manual read was requested, just print the raw value. */
    if (p_data->response_data.att_value.handle == car_data_out_val_handle)
    {
        if (p_data->response_data.att_value.len >= 1)
        {
            task_print("CarDataOut read: 0x%02X\n",
                   p_data->response_data.att_value.p_data[0]);
        }
    }
}
...
{
    if ((p_data->response_data.att_value.handle >= battery_client_app_state.bac_s_handle) &&
        (p_data->response_data.att_value.handle <= battery_client_app_state.bac_e_handle))
    {
        car_data_process_notification(p_data, car_data_out_val_handle, NULL);
    }
}
...
{
    task_print("indication handle:%04x (unexpected for CarData)\r\n",
