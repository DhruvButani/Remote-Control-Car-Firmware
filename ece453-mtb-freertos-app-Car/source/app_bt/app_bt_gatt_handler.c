                    ...
                    uint16_t *p_error_handle)
{
    ...
    switch ( p_attr_req->opcode )
    {
        ...
        case GATT_HANDLE_VALUE_NOTIF:
                    task_print("Notification send complete\n");
             break;
        ...
        case GATT_HANDLE_VALUE_CONF:
            {
                task_print("Indication Confirmation received \n");
                car_sensor_state.flag_indication_sent = FALSE;
            }
             break;

        default:
                task_print("ERROR: Unhandled GATT Connection Request case: %d\n",
                       p_attr_req->opcode);
                gatt_status = WICED_BT_GATT_ERROR;
                break;
    }
                             ...
                             uint16_t *p_error_handle)
{
   ...
   *p_error_handle = p_read_req->handle;
    ...
    task_print("read_handler: conn_id:%d Handle:%x offset:%d len:%d\n ",
                              ...
                              uint16_t *p_error_handle)
{
   ...
   *p_error_handle = p_write_req->handle;
    ...
    task_print("write_handler: conn_id:%d Handle:0x%x offset:%d len:%d\n ",
    ...
    {
        task_print("WARNING: GATT set attr status 0x%x\n", gatt_status);
    }
                                     ...
                                     uint16_t *p_error_handle)
{
    ...
    if (NULL == p_rsp)
    {
        task_print("No memory, len_requested: %d!!\n",len_requested);
    ...
    while (WICED_TRUE)
    {
       *p_error_handle = attr_handle;
        ...
        {
            task_print("found type but no attribute for %d \n",last_handle);
    ...
    if (0 == used_len)
    {
        task_print("attr not found  start_handle: 0x%04x"
...
{
    task_print("Connected to peer device: ");
    print_bd_address(p_status->bd_addr);
    task_print("Connection ID '%d' \n", p_status->conn_id);

    /* Update the connection handler.  Save address of the connected device. */
    car_sensor_state.conn_id = p_status->conn_id;
    memcpy(car_sensor_state.remote_addr, p_status->bd_addr,
...
app_bt_gatt_connection_down(wiced_bt_gatt_connection_status_t *p_status)
{
    ...
    task_print("Peer device disconnected: ");
    print_bd_address(p_status->bd_addr);

    task_print("conn_id:%d reason:%s\n", p_status->conn_id,
           get_bt_gatt_disconn_reason_name(p_status->reason));

    /* Resetting the device info */
    memset(car_sensor_state.remote_addr, 0, BD_ADDR_LEN);
    car_sensor_state.conn_id = 0;
    ...
    {
        task_print("Start advertisement failed: %d\n", result);
    }
    return WICED_BT_GATT_SUCCESS;
}
        ...
        if (app_gatt_db_ext_attr_tbl[i].handle == attr_handle)
        {
            ...
            if (validLen)
            {
                ...
                *  peer can enable or disable notification or indication */
                case HDLD_CARDATA_CARDATAOUT_CLIENT_CHAR_CONFIG:
                    if (len != 2)
                    {
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    }
                    app_cardata_cardataout_client_char_config[0] = p_attr[0];
                    peer_cccd_data[bondindex] = p_attr[0] | (p_attr[1] << 8);
                    rslt = app_bt_update_cccd(peer_cccd_data[bondindex], bondindex);
                    if (CY_RSLT_SUCCESS != rslt)
                    {
                        task_print("Failed to update CCCD Value in Flash! \n");
                    }
                    else{
                        task_print("CCCD value updated in Flash! \n");
                    }
                    break;

                case HDLC_CARDATA_CARDATAIN_VALUE: //Car data recieved from controller
                    if (len != 1)
                    {
                        return WICED_BT_GATT_INVALID_ATTR_LEN;
                    }
                    app_cardata_cardatain[0] = p_attr[0];
                    {
                        uint8_t ev = (uint8_t)app_cardata_cardatain[0];

                        /* Packed layout from controller:
                         *   bit 0: horn         bit 4: brake
                         *   bit 1: pair         bit 5: turn_left
                         *   bit 2: reset        bit 6: turn_right
                         *   bit 3: accelerate   bit 7: gear_sel
                         */
                        task_print("Controller events (0x%02X):\n", ev);
                        task_print("  horn       : %s\n", (ev & (1u << 0)) ? "on" : "off");
                        task_print("  pair       : %s\n", (ev & (1u << 1)) ? "on" : "off");
                        task_print("  reset      : %s\n", (ev & (1u << 2)) ? "on" : "off");
                        task_print("  accelerate : %s\n", (ev & (1u << 3)) ? "on" : "off");
                        task_print("  reverse    : %s\n", (ev & (1u << 4)) ? "on" : "off");
                        task_print("  turn_left  : %s\n", (ev & (1u << 5)) ? "on" : "off");
                        task_print("  turn_right : %s\n", (ev & (1u << 6)) ? "on" : "off");
                        task_print("  gear_sel   : %s\n", (ev & (1u << 7)) ? "on" : "off");

                        if (ev & (1u << 3))
                        {
                            /* accelerate -> CW */
                            task_dc_motor_set_direction(false);
                            task_dc_motor_set_speed(00.0f);
                        }
                        else if (ev & (1u << 4))
                        {
                            /* reverse -> CCW (reverse) */
                            task_dc_motor_set_direction(true);
                            task_dc_motor_set_speed(00.0f);
                        }
                        else
                        {
    ...
    if (!isHandleInTable)
    {
        ...
        switch(attr_handle)
        {
            default:
                 ...
                 * indicated handle */
                task_print("Write Request to Invalid Handle: 0x%x\n", attr_handle);
...
void app_bt_send_message(void)
{
    ...
    task_print("hello_sensor_send_message: CCCD:%d\n", app_cardata_cardataout_client_char_config[0]);

    /* If client has not registered for indication or notification, no action */
    if(0 == app_cardata_cardataout_client_char_config[0])
    {
        return;
    }
    else if(app_cardata_cardataout_client_char_config[0] & GATT_CLIENT_CONFIG_NOTIFICATION)
    {
        status = wiced_bt_gatt_server_send_notification(car_sensor_state.conn_id,
                                                       HDLC_CARDATA_CARDATAOUT_VALUE,
                                                        app_cardata_cardataout_len,
                                                        app_cardata_cardataout,
                                                        NULL);
        task_print("Notification Status: %d \n", status);
    }
    else if(!car_sensor_state.flag_indication_sent)
    {
        car_sensor_state.flag_indication_sent = TRUE;
        status = wiced_bt_gatt_server_send_indication(car_sensor_state.conn_id, HDLC_CARDATA_CARDATAOUT_VALUE, app_cardata_cardataout_len, app_cardata_cardataout, NULL);
...
void app_bt_gatt_increment_notify_value(void)
{
    ...
    int last_byte = app_cardata_cardataout_len - 1 ;
    char c = app_cardata_cardataout[last_byte];
    ...
    app_cardata_cardataout[last_byte] = c;
}
