...
car_sensor_state_t car_sensor_state;
    ...
    switch (event)
    {
        case BTM_ENABLED_EVT:
            ...
            if (WICED_BT_SUCCESS == p_event_data->enabled.status)
            {
                ...
                task_print("Local Bluetooth Address: ");
            ...
            {
                task_print("Bluetooth enable failed \n");
            }
            break;
        ...
        case BTM_PAIRING_COMPLETE_EVT:
            ...
            task_print("Hello sensor, Pairing Complete Reason: %s \n",
        ...
        case BTM_BLE_ADVERT_STATE_CHANGED_EVT:
            ...
            task_print("Advertisement State Change: %s\n",
        ...
        case BTM_PAIRED_DEVICE_LINK_KEYS_UPDATE_EVT:
            ...
            {
                task_print("Successfully Bonded to ");
                print_bd_address(p_event_data->paired_device_link_keys_update.bd_addr);
            }
            else
            {
                task_print("Failed to bond! \n");
            }
            break;

        case  BTM_PAIRED_DEVICE_LINK_KEYS_REQUEST_EVT:
            /* Paired Device Link Keys Request */
            task_print("Paired Device Link keys Request Event for device ");
            ...
            {
                task_print("Device Link Keys not found in the database! \n");
            }
            break;
        ...
        case BTM_ENCRYPTION_STATUS_EVT:
            ...
            task_print("Encryption status changed, BDA:");
            ...
            if(bondindex < BOND_INDEX_MAX)
            {
                ...
                app_cardata_cardataout_client_char_config[0] = peer_cccd_data[bondindex];
                task_print("Bond info present in Flash for device: ");
                print_bd_address(p_event_data->encryption_status.bd_addr);
            }
            else
            {
                task_print("No Bond info present in Flash for device: ");
                print_bd_address(p_event_data->encryption_status.bd_addr);
                bondindex=0;
            }
            break;
        ...
        case BTM_BLE_CONNECTION_PARAM_UPDATE:
            task_print("Connection parameter update status:%d, "
        ...
        case BTM_BLE_PHY_UPDATE_EVT:
            ...
            task_print("Selected TX PHY - %dM\n Selected RX PHY - %dM\n",
                    p_event_data->ble_phy_update_event.tx_phy,
                    p_event_data->ble_phy_update_event.rx_phy);
            break;

        default:
            task_print("Unhandled Bluetooth Management Event: 0x%x %s\n",
...
void app_bt_application_init(void)
{
    ...
    {
        task_print("Keys found in NVRAM, add them to Addr Res DB\n");
    ...
    {
        task_print("Failed to start advertisement! \n");
        CY_ASSERT(0);
    }
}
...
void app_bt_adv_stop_handler(void)
{
    ...
    if ((!car_sensor_state.conn_id) && (!pairing_mode))
    {
        result =  wiced_bt_start_advertisements(BTM_BLE_ADVERT_UNDIRECTED_HIGH,
                                                0,
                                                NULL);
        if(result != WICED_BT_SUCCESS)
            task_print("Advertisement start failed :%d\n", result);
    }
    else
    {
        task_print("Stop Advertisement\n");
    }
