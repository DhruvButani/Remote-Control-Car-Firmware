...
{
    car_sensor_state.timer_count_ms++;
    if(APP_BTN_PRESS_5S == (car_sensor_state.timer_count_ms - btn_press_start) && (is_btn_pressed))
    {
...
void app_bt_hw_init()
{
    ...
    xTaskCreate(button_task,
        ...
        //hw task initializations
    task_dc_motor_init(); //Works
    task_servo_init();

    task_acc_init();//works
    task_tof_init(); //works
    task_ltr390uv_init(); //works

    task_amp_init();
    
    task_sendCarPacket_init();
...
void button_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        {
            is_btn_pressed = TRUE;
            btn_press_start = car_sensor_state.timer_count_ms;
            cyhal_gpio_write(CYBSP_USER_LED2 , CYBSP_LED_STATE_ON);
        }
        else if(0 != btn_press_start)
        {
               ...
               (btn_press_duration <= APP_BTN_PRESS_SHORT_MAX))
            {
                ...
                if (0 == car_sensor_state.conn_id)
                {
                    task_print("Starting Undirected High Advertisement\n");
                ...
                else
                {
                    ...
                    task_print("No. to write: %d\n",
                           car_sensor_state.num_to_send);
                    task_print("flag_indication_sent: %d \n",
                           car_sensor_state.flag_indication_sent);
                    while ((0 != car_sensor_state.num_to_send) &&
                           (FALSE == car_sensor_state.flag_indication_sent))
                    {
                        car_sensor_state.num_to_send--;
                        app_bt_send_message();
                    }
            ...
            {
                task_print("Entering Pairing Mode: Connect, Pair and Bond with a new peer device...\n");
#ifdef PSOC6_BLE
                pairing_mode = TRUE;
                ...
                {
                    task_print("Address resolution list cleared successfully \n");
                }
                else
                {
                    task_print("Failed to clear address resolution list \n");
                }
#endif
                task_print("Starting Undirected High Advertisement\n");
                result = wiced_bt_start_advertisements(BTM_BLE_ADVERT_UNDIRECTED_HIGH,
            ...
            {
                task_print("Button pressed more than 10 seconds,"
                ...
                if (0 == car_sensor_state.conn_id)
                {
                    ...
                    {
                        task_print("Successfully reset kv-store library,"
                               "Please reset the device to generate new keys!\n");
                    }
                    else
                    {
                        task_print("failed to reset kv-store libray\n");
                    }
                ...
                {
                    task_print("Existing connection present, "
